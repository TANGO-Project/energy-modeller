...
import static eu.tango.energymodeller.datasourceclient.KpiList.CPU_INTERUPT_KPI_NAME;
import static eu.tango.energymodeller.datasourceclient.KpiList.CPU_IO_WAIT_KPI_NAME;
import static eu.tango.energymodeller.datasourceclient.KpiList.CPU_NICE_KPI_NAME;
import static eu.tango.energymodeller.datasourceclient.KpiList.CPU_SOFT_IRQ_KPI_NAME;
import static eu.tango.energymodeller.datasourceclient.KpiList.CPU_SPOT_USAGE_KPI_NAME;
import static eu.tango.energymodeller.datasourceclient.KpiList.CPU_STEAL_KPI_NAME;
import static eu.tango.energymodeller.datasourceclient.KpiList.CPU_SYSTEM_KPI_NAME;
import static eu.tango.energymodeller.datasourceclient.KpiList.CPU_USER_KPI_NAME;
...
import eu.tango.energymodeller.datasourceclient.hostvmfilter.NameBeginsFilter;
import eu.tango.energymodeller.datasourceclient.hostvmfilter.ZabbixHostVMFilter;
...
public class ZabbixDataSourceAdaptor implements HostDataSource {
    ...
    public static void main(String[] args) {
        ...
        List<eu.tango.energymodeller.types.energyuser.Host> hosts = adaptor.getHostList();
        for (eu.tango.energymodeller.types.energyuser.Host host : hosts) {
     ...
     */
    @Override
    public eu.tango.energymodeller.types.energyuser.Host getHostByName(String hostname) {
     ...
     */
    @Override
    public List<eu.tango.energymodeller.types.energyuser.Host> getHostList() {
        List<Host> hostsList = client.getAllHosts();
        ArrayList<eu.tango.energymodeller.types.energyuser.Host> hosts = new ArrayList<>();
    ...
    public List<eu.tango.energymodeller.types.energyuser.Host> getHostList(String groupName) {
        List<Host> hostsList = client.getAllHosts();
        ArrayList<eu.tango.energymodeller.types.energyuser.Host> hosts = new ArrayList<>();
     ...
     */
    private eu.tango.energymodeller.types.energyuser.Host convert(Host host) {
        String hostname = host.getHost();
        int hostId = Integer.parseInt(host.getHostid());
        eu.tango.energymodeller.types.energyuser.Host answer = new eu.tango.energymodeller.types.energyuser.Host(hostId, hostname);
     ...
     */
    private eu.tango.energymodeller.types.energyuser.Host getHostByName(String hostName, List<Host> allHosts) {
        if (allHosts == null) {
     ...
     */
    @Override
    public HostMeasurement getHostData(eu.tango.energymodeller.types.energyuser.Host host) {
        ArrayList<eu.tango.energymodeller.types.energyuser.Host> hostList = new ArrayList<>();
        hostList.add(host);
     ...
     */
    @Override
    public List<HostMeasurement> getHostData(List<eu.tango.energymodeller.types.energyuser.Host> hostList) {
        if (hostList.isEmpty()) {
            return new ArrayList<>();
        }
        HashMap<Integer, HostMeasurement> hostMeasurements = new HashMap<>();
        for (eu.tango.energymodeller.types.energyuser.Host host : hostList) {
    ...
    public List<VmMeasurement> getVmData(List<VmDeployed> vmList) {
        ...
        for (Item item : itemsList) {
             ...
             * Note: Additional hosts could be discovered using the following
             * code: host = new HostMeasurement(new
             * eu.tango.energymodeller.types.energyuser.Host(hostID,
             * "UNKNOWN")); hostMeasurements.put(hostID, host);
             *
             * This is the case if the host id in the metric does not match any
             * of the named hosts.
     ...
     */
    @Override
    public CurrentUsageRecord getCurrentEnergyUsage(eu.tango.energymodeller.types.energyuser.Host host) {
     ...
     */
    @Override
    public double getLowestHostPowerUsage(eu.tango.energymodeller.types.energyuser.Host host) {
     ...
     */
    @Override
    public double getCpuUtilisation(eu.tango.energymodeller.types.energyuser.Host host, int lastNSeconds) {
